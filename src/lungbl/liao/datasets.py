...
from lungbl.liao.layers import nms,iou
from lungbl.datamodule import PandasDataset, Item
from lungbl.liao.config import config as LIAO_CONFIG
from lungbl.utils.tabular import format_datetime_str

class DataBowl3Classifier(PandasDataset):
    def __init__(self,
        df,
        data_cache,
        config: dict=LIAO_CONFIG,
        phase: str='train',
        **kwargs):
        ...
        datadir = self.data_cache.imgprep_data
        bboxpath = self.data_cache.imgbbox_data
        self.phase = phase
        self.candidate_box = []
        self.pbb_label = []
        self.subj_name = []

        idcs = self.df["pid"] + "time" + self.df['scandate'].apply(lambda x: format_datetime_str(x, format=self.date_format))
        idcs = idcs.tolist()
        self.filenames = [os.path.join(datadir, '%s_clean.npy' % idx.split('-')[0]) for idx in idcs]
        if self.phase!='test':
            self.yset = self.df[self.label].to_numpy().astype('int')
...
#             if idx.startswith()
    ...
    def __getitem__(self, idx, split=None):
        pid, scandate, *_ = self.subj_name[idx].split("time")
        ...
        if self.phase!='test':
            ...
            return Item(
                pid=pid, scandate=scandate, 
                data=[torch.from_numpy(croplist).float(), torch.from_numpy(coordlist).float()],
