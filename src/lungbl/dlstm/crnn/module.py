...
from lungbl.dlstm.crnn.functional import AutogradConvRNN, _conv_cell_helper, ConvNdWithSamePadding
from lungbl.dlstm.crnn.utils import _single, _pair, _triple
    ...
    def check_forward_hidden(self, input, hx, hidden_label=''):
        ...
        if hx.size(1) != self.out_channels:
            raise RuntimeError(
                "hidden{} has inconsistent hidden_size: got {}, expected {}".format(
                    hidden_label, hx.size(1), self.out_channels))        
      #, time_dis, hidden, w_ih, w_hh, b_ih=None, b_hh=None, linear_func=None  
...
class TimeLSTMCell(torch.nn.Module):
    """ 
    This the part that I want to modify to create the distance gate. 
    option 1: forgetgate =F.sigmod( a * exp(- max(b-cx,0)) * forgetgate)
    """
    def __init__(self, mode: str,
        ...
        if bias:
            self.bias_ih = Parameter(torch.Tensor(gate_size))
            self.bias_hh = Parameter(torch.Tensor(gate_size))
            self.bias_t = Parameter(torch.Tensor(out_channels))

        else:
            self.register_parameter('bias_ih', None)
            self.register_parameter('bias_hh', None)

        self.weight_xt = Parameter(torch.Tensor(out_channels, in_channels // groups, *self.kernel_size))
        self.weight_tt = Parameter(torch.Tensor(1))  # here the "1" represents the number of time interval.
        self.weight_to = Parameter(torch.Tensor(1))
        self.reset_parameters()
    ...
    def forward(self, input, time_dis, hidden = None): 
        ...
        cell = _conv_cell_helper(
            ...
            groups=self.groups) 
        return cell(
                input, time_dis, hidden, self.weight_xt, self.weight_tt, self.weight_to, self.bias_t,
                self.weight_ih, self.weight_hh,
                self.bias_ih, self.bias_hh,
            )    
    
class TLSTMCell(torch.nn.Module):
    ...
    def __init__(self, mode: str,
        ...
        nn.init.uniform_(self.a.data,a=0.2,b= 0.21)  # default 0.5
