...
from lungbl.dlstm.rnn.functional import disLSTMCell, oLSTMCell
...
class LSTMCell(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, bias=True): # tested 0226
    ...
    def forward(self, input, hx=None):
        ...
        return oLSTMCell(input, hx,self.weight_ih, self.weight_hh,self.bias_ih, self.bias_hh )    

class dLSTMCell(torch.nn.Module):

    def __init__(self, input_size, hidden_size, mode, bias=True):
        ...
        nn.init.uniform_(self.a.data,a=0.89,b=0.90)
        nn.init.uniform_(self.b.data,a=0.,b=1)
        nn.init.uniform_(self.c.data,a=1.0,b=1.01)
    ...
    def forward(self, input, time_dis, hx=None):
        self.check_forward_input(input)
        if hx is None:
            hx = input.new_zeros(input.size(0), self.hidden_size, requires_grad=False)
            hx = (hx, hx)
        ...
        self.check_forward_hidden(input, hx[1], '[1]')
        return disLSTMCell(     
            input, time_dis, hx, self.a, self.b, self.c,
            self.weight_ih, self.weight_hh,
