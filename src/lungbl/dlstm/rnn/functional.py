...
def oLSTMCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
#     if input.is_cuda:
...
def disLSTMCell(input, time_dis, hidden, a, b, c, w_ih, w_hh, b_ih=None, b_hh=None, mode = 'dis_exp'):
#     if input.is_cuda:   # it only affect the speed when commit this. 
#         igates = F.linear(input, w_ih)
#         hgates = F.linear(hidden[0], w_hh)
#         state = fusedBackend.LSTMFused.apply
#         return state(igates, hgates, hidden[1]) if b_ih is None else state(igates, hgates, hidden[1], b_ih, b_hh)
    ...
    if mode == 'dis_exp':
        ...
        #print ('a, c: ', a, c)
        #print ('coff: ', coff)
        #print ('coff shape, and forgetgate shape',coff.shape, forgetgate.shape)
        coff = coff.unsqueeze(1).expand_as(forgetgate)
        forgetgate = coff * F.sigmoid(forgetgate)
        
#     if mode == 'dis_explog':
#         coff =  a.cuda() * torch.exp( - c.cuda() * nn.sigmoid (logtime_dis) 
#         #print ('a, c: ', a, c)
#         #print ('coff: ', coff)
#         #print ('coff shape, and forgetgate shape',coff.shape, forgetgate.shape)
#         coff = coff.unsqueeze(1).expand_as(forgetgate)
#         forgetgate = coff * F.sigmoid(forgetgate)
    ...
    if mode == 'infor_exp':
        ...
        coff = coff.unsqueeze(1).expand_as(forgetgate)
        forgetgate = coff * F.sigmoid(forgetgate)
        ingate = coff * ingate     # add 0223
       
        
    if mode == 'dis_ploy_1': 
        coff =  a * torch.max(torch.tensor(1 -  c.cuda() * time_dis), torch.tensor([0.]))
        coff = coff.unsqueeze(1).expand_as(forgetgate)
        forgetgate = coff * F.sigmoid(forgetgate)
        #ingate = coff * ingate     # add 0213
        
#     if mode == 'dis_ploy_2':
#         forgetgate =  a* torch.pow(torch.max(torch.tensor(1 - time_dis).cuda(), torch.tensor([0.]).cuda()), 2) * F.sigmoid(forgetgate)
#     if mode == 'dis_ploy_0.5':
#         forgetgate =  a* torch.pow(torch.max(torch.tensor(1 - time_dis).cuda(), torch.tensor([0.]).cuda()), 0.5) * F.sigmoid(forgetgate)
    # exp(-x), log(1 + exp(-x)), log(1 + exp(-x^n))
